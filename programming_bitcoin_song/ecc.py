...
# Define the constants for the Bitcoint FieldElements and Elliptic Curve Generator Point
A = 0
B = 7
P = 2**256 - 2**32 - 977
...
class Point:
    ...
    def __add__(self, other):
        ...
        # Case 0: self is the point at infinity, return other
        # Note:  only have to test the case of self.x is None, not both
        #        because only way a valid point could be created is if self.x is None and self.y is None.
        if self.x is None:
            return other
...
class S256Point(Point):
    ...
    def sec(self, compressed=True):
        ...
        if compressed:
            # For every (x,y) that satisfies the bitcoin elliptic curve equation y^2 = x^3 +7, the (x, -y) solves it too
            # Any x,y are elements of the finite field over p, which is prime.  Thus
            # -y%p = (p-y)%p.  This means of y and -y, one is even the other is odd
...
#  Definte the constant G Generator Point
G = S256Point(
...
class PrivateKey:
    ...
    def wif(self, compressed=True, testnet=False):
        ...
        whole_thing = prefix + secret_bytes + suffix
        print(len(whole_thing))
        return encode_base58_checksum(prefix + secret_bytes + suffix)  # a string
