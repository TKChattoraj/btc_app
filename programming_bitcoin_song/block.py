...
class Block:
    command = b'block'
    def __init__(self, version, prev_block, merkle_root, timestamp, bits, nonce, tx_hashes=None ):  #  if want to build in the attributes, include these in the init function:  block_serialized=None, block_hash=None
        ...
        # if you want the following Block attributes:
        # self.block_serialized = block_serialized
        # self.block_hash = block_hash

    @classmethod
    def parse(cls, s):
        # s will be a byte stream
    ...
    def serialize(self):
        ...
        result += self.nonce
        return result
        #  If you wanted to build in a block_serialized attribute:
        #self.block_serialized = result

    def hash(self):
        h = hash256(self.serialize())
        return h[::-1]
        #  If you want to build in a block_hash attribute:
        #self.block_hash = h[::-1]
    ...
    def difficulty(self):
        ...
        #lowest_difficulty_bits = bytes.fromhex('ffff001d')
        #lowest_difficulty_target = bits_to_target(lowest_difficulty_bits)
        lowest_difficulty_target = 0xffff*256**(0x1d - 3)
        difficulty = lowest_difficulty_target/self.target()
        return difficulty
        
    def check_pow(self):
        '''Returns whether this block satisfies proof of work'''

        if little_endian_to_int(hash256(self.serialize())) < self.target():
            return True
        else:
            return False
    ...
    def time_differential(self, first_block):
...
class BlockTest(TestCase):
    ...
    def test_serialize(self):
        ...
        #self.assertEqual(block.block_serialized, block_raw)

    def test_hash(self):
        block_raw = bytes.fromhex('020000208ec39428b17323fa0ddec8e887b4a7c53b8c0a0a220cfd0000000000000000005b0750fce0a889502d40508d39576821155e9c9e3f5c3157f961db38fd8b25be1e77a759e93c0118a4ffd71d')
        stream = BytesIO(block_raw)
        block = Block.parse(stream)
        print("Asserting hash strings equal:")
        self.assertEqual(block.hash().hex(), '0000000000000000007e9e4c586439b0cdbe13b1370bdd9435d76a644d047523')
        print("Asserting hash bytes are equal")
