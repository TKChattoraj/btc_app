...
class MerkleBlock:
    ...
    def parse(cls, s):
        ...
        number_txs_in_block = little_endian_to_int(s.read(4))
        number_hashes = read_varint(s)
        hashes = []
        for _ in range(number_hashes):
            hashes.append(s.read(32)[::-1])  #hashes come in as little endian--reverse to make big endian
        flag_field_length = read_varint(s)
        flags_bytes = s.read(flag_field_length)
        #flags = bytes_to_bit_field(flag_bytes)
        return MerkleBlock(version, prev_block, merkle_root, timestamp, bits, nonce, number_txs_in_block, hashes, flags_bytes )
