...
def valid_sig(sig, pub_key_points, z):
    
    for point in pub_key_points:
        if point.verify(z, sig):
            #need remove that point from pub_key_point--assuming that point does not repeat in pub_key_points
            pub_key_points.remove(point)
            return True
    return False
